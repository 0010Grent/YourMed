...
本项目的 RAG 向量库使用 Chroma 持久化到 `app/rag/kb_store/`（默认 `chroma.sqlite3`）。
...
| 两阶段检索 | 只做向量 top_k | 向量 top_n（默认 30）+ 可选 BCE rerank | 调整 `RAG_TOP_N`/`RAG_USE_RERANKER` 对比结果 |
| 证据契约 | 字段可能不固定 | 固定字段：eid/text/source/chunk_id/score/rerank_score/metadata | 运行测试 `tests/test_rag_retrieve_contract.py` |
| 可观测性 | 打印信息不统一 | `RAG_DEBUG=1` 输出 collection/count/device/model | `GET /v1/rag/stats` |
